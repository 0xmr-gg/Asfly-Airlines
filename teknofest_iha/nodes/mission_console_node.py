...
class MissionConsoleNode(Node):
    ...
    def _status_block(self) -> str:
        ...
        perception_validated = bool(selected.get("perception_validated", False))
        validation_status = "VALIDATED" if perception_validated else ("UNVALIDATED" if selected else "WAIT")
        ...
        return "\n".join(
            [
                ...
                f"[VISION ] opencv={opencv_status}  validation={validation_status}",
    ...
    def _stateflow_line(self) -> str:
        ...
        active_phase = self._active_phase(mission, fusion_state, opencv_seen, perception_validated, release_gate)
        ...
        validation = "VALIDATED" if perception_validated else ("UNVALIDATED" if selected else "WAIT")
        ...
        return (
            ...
            f"OpenCV={opencv} VALIDATION={validation} FUSION={fusion} ALIGN={alignment} "
    ...
    def _perception_line(self) -> str:
        ...
        return f"FRAME #{frame_id} OpenCV={cv_summary}"
    ...
    def _active_phase(
        ...
        if mission == "SEARCH_TARGET":
            ...
            if not perception_validated:
                return "OpenCV TARGET DETECTION -> PERCEPTION VALIDATION"
    ...
    @staticmethod
    def _tracking_message(state: str, perception_validated: bool, release_gate: bool) -> str:
        if release_gate:
        ...
        if state == "UNSTABLE":
            return "KARARSIZ: OpenCV doğrulaması zayıf"
        if perception_validated:
            return "Algı doğrulandı"
