...
This detector separates red_square and blue_square using color masks and
geometric filters.
"""
...
def annotate_detection(det, detection_mode, observation_valid, shape_valid, validation_reason, shape_score=None):
    det.update(
        {
    ...
    if shape_score is not None:
        det["shape_score"] = float(shape_score)
    return det
    ...
    def detect(self, frame_bgr, frame_id=0):
        ...
        for spec in self.target_specs:
            ...
            if result is not None:
                ...
                annotate_detection(det, "contour", True, True, "ok", shape_score=shape_score)
            ...
            if track_ok and state["track_frames"] < config.MAX_TRACK_FRAMES:
                ...
                annotate_detection(det, "tracker", False, False, "tracker_fallback")
                detections.append(det)
            ...
            if (
                ...
                annotate_detection(det, "kalman", False, False, "kalman_prediction")
                detections.append(det)
