...
def test_drop_target_done_enters_post_drop_hover():
    machine = MissionStateMachine(10.0, 0.5, 1.0)
    machine.state = MissionState.DROP_TARGET

    assert machine.update(MissionInputs(drop_done=True)) == MissionState.POST_DROP_HOVER


def test_drop_target_lost_target_returns_to_search():
    machine = MissionStateMachine(10.0, 0.5, 1.0)
    machine.state = MissionState.DROP_TARGET

    assert machine.update(MissionInputs()) == MissionState.SEARCH_TARGET


def test_drop_target_decentered_returns_to_align():
    machine = MissionStateMachine(10.0, 0.5, 1.0)
    machine.state = MissionState.DROP_TARGET

    inputs = MissionInputs(target={"target_type": "blue_square"}, target_centered=False, target_locked=True)

    assert machine.update(inputs) == MissionState.TARGET_ALIGN


def test_drop_target_lock_lost_returns_to_verify():
    machine = MissionStateMachine(10.0, 0.5, 1.0)
    machine.state = MissionState.DROP_TARGET

    inputs = MissionInputs(target={"target_type": "blue_square"}, target_centered=True, target_locked=False)

    assert machine.update(inputs) == MissionState.TARGET_VERIFY


def test_drop_target_stays_when_drop_conditions_still_hold():
    machine = MissionStateMachine(10.0, 0.5, 1.0)
    machine.state = MissionState.DROP_TARGET

    inputs = MissionInputs(target={"target_type": "blue_square"}, target_centered=True, target_locked=True)

    assert machine.update(inputs) == MissionState.DROP_TARGET
